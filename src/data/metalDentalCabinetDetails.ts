import type { HighEndDentalChairDetail } from "./highEndDentalChairDetails";

export const gA3: HighEndDentalChairDetail = {
  "slug": "g-a3",
  "model": "G-A3",
  "title": "G-A3 Mobile Dental Cabinet",
  "seoDescription": "G-A3 five-drawer mobile dental cabinet with a stainless-steel body, tempered glass countertop, adjustable drawer storage and brake wheels.",
  "productType": "Compact mobile dental cabinet",
  "tagline": "A five-drawer mobile dental cabinet for daily clinic organization, combining a glass countertop, stainless-steel cabinet body, adjustable drawer storage and smooth brake wheels.",
  "gallery": [
    {
      "src": "/images/products/dental-cabinet/g-a3/gallery-1.webp",
      "width": 800,
      "height": 800,
      "label": "Front view",
      "alt": "G-A3 mobile dental cabinet — front view"
    },
    {
      "src": "/images/products/dental-cabinet/g-a3/gallery-2.webp",
      "width": 800,
      "height": 800,
      "label": "Open drawer organizer",
      "alt": "G-A3 mobile dental cabinet — open drawer organizer"
    },
    {
      "src": "/images/products/dental-cabinet/g-a3/gallery-3.webp",
      "width": 800,
      "height": 800,
      "label": "Drawers open view",
      "alt": "G-A3 mobile dental cabinet — drawers open view"
    },
    {
      "src": "/images/products/dental-cabinet/g-a3/gallery-4.webp",
      "width": 800,
      "height": 800,
      "label": "White front view",
      "alt": "G-A3 mobile dental cabinet — white front view"
    }
  ],
  "quickSpecs": [
    {
      "label": "Size",
      "value": "500 × 500 × 800 mm"
    },
    {
      "label": "Countertop",
      "value": "Tempered glass"
    },
    {
      "label": "Storage",
      "value": "Five drawers with adjustable divider"
    },
    {
      "label": "Mobility",
      "value": "Silent wheels with brake"
    }
  ],
  "trustHighlights": [
    "Five-drawer storage layout",
    "Easy-clean glass top",
    "Mobile cabinet with brake wheels"
  ],
  "sellingPointHeading": "Why clinics choose the G-A3",
  "sellingPoints": [
    {
      "title": "Five-drawer organized storage",
      "description": "Keep instruments, consumables and small accessories separated in a clean drawer layout for daily clinic work."
    },
    {
      "title": "Easy-clean glass countertop",
      "description": "The tempered glass top gives the cabinet a smooth, stain-resistant surface that is simple to wipe down between procedures."
    },
    {
      "title": "Mobile cabinet for flexible placement",
      "description": "Silent universal wheels with brake make it easy to move the cabinet into position and keep it stable during use."
    },
    {
      "title": "Durable stainless-steel cabinet body",
      "description": "The high-quality stainless-steel body and coated finish support long-term use in busy treatment rooms."
    }
  ],
  "videoPlaceholder": {
    "title": "",
    "note": ""
  },
  "featureHeading": "Built for clean, organized dental storage",
  "features": [
    {
      "src": "/images/products/dental-cabinet/g-a3/feature-1.webp",
      "width": 266,
      "height": 267,
      "label": "feature-1",
      "alt": "G-A3 feature 1",
      "eyebrow": "SILENT UNIVERSAL WHEELS",
      "title": "Flexible placement in the treatment room",
      "description": "The mobile wheel design makes the cabinet easy to position beside the chair, while the stable wheel structure supports daily movement and parking.",
      "bullets": [
        "Easy room repositioning",
        "Smooth movement beside the chair",
        "Practical support for compact clinics"
      ]
    },
    {
      "src": "/images/products/dental-cabinet/g-a3/feature-2.webp",
      "width": 267,
      "height": 267,
      "label": "feature-2",
      "alt": "G-A3 feature 2",
      "eyebrow": "KEY LOCK FOR SECURE STORAGE",
      "title": "Protect frequently used clinic items",
      "description": "The drawer lock helps keep selected instruments, materials and personal items stored safely when the cabinet is not in use.",
      "bullets": [
        "Lockable storage area",
        "Better material management",
        "Suitable for shared treatment rooms"
      ]
    },
    {
      "src": "/images/products/dental-cabinet/g-a3/feature-3.webp",
      "width": 267,
      "height": 267,
      "label": "feature-3",
      "alt": "G-A3 feature 3",
      "eyebrow": "OPTIONAL HANDLE COLORS",
      "title": "Color details for different clinic styles",
      "description": "Multiple handle color options allow the cabinet to match different treatment-room interiors while keeping a clean white cabinet body.",
      "bullets": [
        "Multiple color choices",
        "Easy clinic color matching",
        "Fresh visual detail"
      ]
    },
    {
      "src": "/images/products/dental-cabinet/g-a3/feature-4.webp",
      "width": 266,
      "height": 267,
      "label": "feature-4",
      "alt": "G-A3 feature 4",
      "eyebrow": "SMOOTH DRAWER RAIL SYSTEM",
      "title": "Stable drawer movement for daily use",
      "description": "The drawer rail structure supports smooth opening and closing, helping staff access stored items quickly during clinic preparation.",
      "bullets": [
        "Smooth drawer operation",
        "Organized storage access",
        "Built for repeated daily use"
      ]
    },
    {
      "src": "/images/products/dental-cabinet/g-a3/feature-5.webp",
      "width": 267,
      "height": 267,
      "label": "feature-5",
      "alt": "G-A3 feature 5",
      "eyebrow": "ADJUSTABLE DRAWER DIVIDER",
      "title": "Flexible compartments for small instruments",
      "description": "The internal divider separates tools and consumables into clear compartments, making small items easier to find and arrange.",
      "bullets": [
        "Adjustable compartment layout",
        "Clear separation for small items",
        "Cleaner drawer organization"
      ]
    }
  ],
  "colors": [
    {
      "code": "Blue",
      "name": "Blue",
      "image": "/images/products/dental-cabinet/g-a3/swatch-blue.webp",
      "swatch": "/images/products/dental-cabinet/g-a3/swatch-blue.webp",
      "alt": "Blue cabinet finish swatch",
      "description": ""
    },
    {
      "code": "Golden",
      "name": "Golden",
      "image": "/images/products/dental-cabinet/g-a3/swatch-golden.webp",
      "swatch": "/images/products/dental-cabinet/g-a3/swatch-golden.webp",
      "alt": "Golden cabinet finish swatch",
      "description": ""
    },
    {
      "code": "Red",
      "name": "Red",
      "image": "/images/products/dental-cabinet/g-a3/swatch-red.webp",
      "swatch": "/images/products/dental-cabinet/g-a3/swatch-red.webp",
      "alt": "Red cabinet finish swatch",
      "description": ""
    },
    {
      "code": "Green",
      "name": "Green",
      "image": "/images/products/dental-cabinet/g-a3/swatch-green.webp",
      "swatch": "/images/products/dental-cabinet/g-a3/swatch-green.webp",
      "alt": "Green cabinet finish swatch",
      "description": ""
    },
    {
      "code": "White",
      "name": "White",
      "image": "/images/products/dental-cabinet/g-a3/swatch-white.webp",
      "swatch": "/images/products/dental-cabinet/g-a3/swatch-white.webp",
      "alt": "White cabinet finish swatch",
      "description": ""
    },
    {
      "code": "Black",
      "name": "Black",
      "image": "/images/products/dental-cabinet/g-a3/swatch-black.webp",
      "swatch": "/images/products/dental-cabinet/g-a3/swatch-black.webp",
      "alt": "Black cabinet finish swatch",
      "description": ""
    }
  ],
  "colorHeading": "Clean finishes for dental cabinet setups",
  "colorIntroduction": "The G-A3 combines a white coated stainless-steel body with a tempered glass countertop. Select a finish to coordinate with your clinic interior.",
  "specifications": [
    {
      "label": "Model",
      "value": "G-A3"
    },
    {
      "label": "Product type",
      "value": "Mobile dental cabinet"
    },
    {
      "label": "Size",
      "value": "500 x 500 x 800 mm"
    },
    {
      "label": "Countertop",
      "value": "Tempered glass"
    },
    {
      "label": "Cabinet body",
      "value": "High-quality stainless steel"
    },
    {
      "label": "Finish",
      "value": "High-temperature coated white surface"
    },
    {
      "label": "Storage",
      "value": "Five drawers"
    },
    {
      "label": "Drawer divider",
      "value": "Built-in adjustable divider"
    },
    {
      "label": "Wheels",
      "value": "Silent universal wheels with brake"
    },
    {
      "label": "Guide rail",
      "value": "Professional-grade drawer rail"
    },
    {
      "label": "Optional drawer color",
      "value": "Multiple color choices"
    },
    {
      "label": "Certification",
      "value": "CE certificate available"
    }
  ],
  "resources": [
    {
      "label": "Dental Cabinet Catalog",
      "href": "/images/products/dental-cabinet/g-a3/cabinet-catalog-3.pdf",
      "meta": "PDF · 18.1 MB"
    },
    {
      "label": "Dental Cabinet Collection · 2024",
      "href": "/images/products/dental-cabinet/g-a3/cabinet-catalog-4.pdf",
      "meta": "PDF · 86.1 MB"
    }
  ],
  "caseHeading": "G-A3 in finished clinic scenes",
  "caseIntroduction": "Explore compact mobile storage in different treatment-room settings.",
  "cases": [
    {
      "src": "/images/products/dental-cabinet/g-a3/case-1.webp",
      "width": 1000,
      "height": 1000,
      "label": "case-1",
      "alt": "G-A3 case 1",
      "title": "Compact cabinet beside the treatment area",
      "description": "The G-A3 provides mobile storage beside the dental chair, keeping instruments and materials close while maintaining a clean room layout."
    },
    {
      "src": "/images/products/dental-cabinet/g-a3/case-2.webp",
      "width": 800,
      "height": 800,
      "label": "case-2",
      "alt": "G-A3 case 2",
      "title": "Bright clinic storage corner",
      "description": "Placed near the window, the cabinet adds organized drawer storage without taking up too much floor space in the treatment room."
    }
  ],
  "faq": [
    {
      "question": "What is this mobile dental cabinet used for?",
      "answer": "The G-A3 is a mobile dental cabinet for storing instruments, consumables and small clinic accessories beside the treatment area."
    },
    {
      "question": "What is the cabinet size?",
      "answer": "The listed size is 500 x 500 x 800 mm."
    },
    {
      "question": "What material is used for the countertop?",
      "answer": "The cabinet uses a tempered glass countertop for a smooth, easy-clean work surface."
    },
    {
      "question": "Can the cabinet be moved?",
      "answer": "Yes. It uses silent universal wheels with brake support for flexible placement and stable parking."
    },
    {
      "question": "Are drawer colors optional?",
      "answer": "Yes. The source material shows multiple optional drawer colors for clinic matching."
    }
  ],
  "relatedProducts": []
};

export const cabinetFilmSwatches = [
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb02.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb02",
    "alt": "G-A3 film hb02",
    "code": "HB02"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb03.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb03",
    "alt": "G-A3 film hb03",
    "code": "HB03"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb10.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb10",
    "alt": "G-A3 film hb10",
    "code": "HB10"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb04.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb04",
    "alt": "G-A3 film hb04",
    "code": "HB04"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb09.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb09",
    "alt": "G-A3 film hb09",
    "code": "HB09"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb22.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb22",
    "alt": "G-A3 film hb22",
    "code": "HB22"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb29.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb29",
    "alt": "G-A3 film hb29",
    "code": "HB29"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb11.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb11",
    "alt": "G-A3 film hb11",
    "code": "HB11"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb08.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb08",
    "alt": "G-A3 film hb08",
    "code": "HB08"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb07.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb07",
    "alt": "G-A3 film hb07",
    "code": "HB07"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb23.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb23",
    "alt": "G-A3 film hb23",
    "code": "HB23"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd01.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd01",
    "alt": "G-A3 film hd01",
    "code": "HD01"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb28.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb28",
    "alt": "G-A3 film hb28",
    "code": "HB28"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb30.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb30",
    "alt": "G-A3 film hb30",
    "code": "HB30"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc85.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc85",
    "alt": "G-A3 film hc85",
    "code": "HC85"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc100.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc100",
    "alt": "G-A3 film hc100",
    "code": "HC100"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc88.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc88",
    "alt": "G-A3 film hc88",
    "code": "HC88"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc89.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc89",
    "alt": "G-A3 film hc89",
    "code": "HC89"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc90.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc90",
    "alt": "G-A3 film hc90",
    "code": "HC90"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc92.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc92",
    "alt": "G-A3 film hc92",
    "code": "HC92"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc94.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc94",
    "alt": "G-A3 film hc94",
    "code": "HC94"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc95.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc95",
    "alt": "G-A3 film hc95",
    "code": "HC95"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc96.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc96",
    "alt": "G-A3 film hc96",
    "code": "HC96"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc93.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc93",
    "alt": "G-A3 film hc93",
    "code": "HC93"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd26.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd26",
    "alt": "G-A3 film hd26",
    "code": "HD26"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd32.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd32",
    "alt": "G-A3 film hd32",
    "code": "HD32"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd02.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd02",
    "alt": "G-A3 film hd02",
    "code": "HD02"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd11.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd11",
    "alt": "G-A3 film hd11",
    "code": "HD11"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd16.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd16",
    "alt": "G-A3 film hd16",
    "code": "HD16"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc59.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc59",
    "alt": "G-A3 film hc59",
    "code": "HC59"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd38.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd38",
    "alt": "G-A3 film hd38",
    "code": "HD38"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd37.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd37",
    "alt": "G-A3 film hd37",
    "code": "HD37"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd39.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd39",
    "alt": "G-A3 film hd39",
    "code": "HD39"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc64.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc64",
    "alt": "G-A3 film hc64",
    "code": "HC64"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc71.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc71",
    "alt": "G-A3 film hc71",
    "code": "HC71"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc56.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc56",
    "alt": "G-A3 film hc56",
    "code": "HC56"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc57.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc57",
    "alt": "G-A3 film hc57",
    "code": "HC57"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc54.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc54",
    "alt": "G-A3 film hc54",
    "code": "HC54"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc55.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc55",
    "alt": "G-A3 film hc55",
    "code": "HC55"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc60.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc60",
    "alt": "G-A3 film hc60",
    "code": "HC60"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc61.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc61",
    "alt": "G-A3 film hc61",
    "code": "HC61"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc62.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc62",
    "alt": "G-A3 film hc62",
    "code": "HC62"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc50.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc50",
    "alt": "G-A3 film hc50",
    "code": "HC50"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc66.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc66",
    "alt": "G-A3 film hc66",
    "code": "HC66"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc74.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc74",
    "alt": "G-A3 film hc74",
    "code": "HC74"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc73.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc73",
    "alt": "G-A3 film hc73",
    "code": "HC73"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc78.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc78",
    "alt": "G-A3 film hc78",
    "code": "HC78"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc773.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc773",
    "alt": "G-A3 film hc773",
    "code": "HC773"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb32.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb32",
    "alt": "G-A3 film hb32",
    "code": "HB32"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc86.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc86",
    "alt": "G-A3 film hc86",
    "code": "HC86"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc82.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc82",
    "alt": "G-A3 film hc82",
    "code": "HC82"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc83.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc83",
    "alt": "G-A3 film hc83",
    "code": "HC83"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc84.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc84",
    "alt": "G-A3 film hc84",
    "code": "HC84"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb31.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb31",
    "alt": "G-A3 film hb31",
    "code": "HB31"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb29.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb29",
    "alt": "G-A3 film hb29",
    "code": "HB29"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc101.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc101",
    "alt": "G-A3 film hc101",
    "code": "HC101"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-page10.webp",
    "width": 500,
    "height": 500,
    "label": "film-page10",
    "alt": "G-A3 film page10",
    "code": "PAGE10"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-page10.webp",
    "width": 500,
    "height": 500,
    "label": "film-page10",
    "alt": "G-A3 film page10",
    "code": "PAGE10"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-page10.webp",
    "width": 500,
    "height": 500,
    "label": "film-page10",
    "alt": "G-A3 film page10",
    "code": "PAGE10"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-page10.webp",
    "width": 500,
    "height": 500,
    "label": "film-page10",
    "alt": "G-A3 film page10",
    "code": "PAGE10"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc37.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc37",
    "alt": "G-A3 film hc37",
    "code": "HC37"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd35.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd35",
    "alt": "G-A3 film hd35",
    "code": "HD35"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb20.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb20",
    "alt": "G-A3 film hb20",
    "code": "HB20"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb21.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb21",
    "alt": "G-A3 film hb21",
    "code": "HB21"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc52.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc52",
    "alt": "G-A3 film hc52",
    "code": "HC52"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc35.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc35",
    "alt": "G-A3 film hc35",
    "code": "HC35"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb16.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb16",
    "alt": "G-A3 film hb16",
    "code": "HB16"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb15.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb15",
    "alt": "G-A3 film hb15",
    "code": "HB15"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb19.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb19",
    "alt": "G-A3 film hb19",
    "code": "HB19"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc30.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc30",
    "alt": "G-A3 film hc30",
    "code": "HC30"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc31.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc31",
    "alt": "G-A3 film hc31",
    "code": "HC31"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc41.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc41",
    "alt": "G-A3 film hc41",
    "code": "HC41"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc39.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc39",
    "alt": "G-A3 film hc39",
    "code": "HC39"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc40.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc40",
    "alt": "G-A3 film hc40",
    "code": "HC40"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc44.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc44",
    "alt": "G-A3 film hc44",
    "code": "HC44"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hb17.webp",
    "width": 500,
    "height": 500,
    "label": "film-hb17",
    "alt": "G-A3 film hb17",
    "code": "HB17"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc51.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc51",
    "alt": "G-A3 film hc51",
    "code": "HC51"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc53.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc53",
    "alt": "G-A3 film hc53",
    "code": "HC53"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc25.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc25",
    "alt": "G-A3 film hc25",
    "code": "HC25"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd31.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd31",
    "alt": "G-A3 film hd31",
    "code": "HD31"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd22.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd22",
    "alt": "G-A3 film hd22",
    "code": "HD22"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd23.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd23",
    "alt": "G-A3 film hd23",
    "code": "HD23"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hd21.webp",
    "width": 500,
    "height": 500,
    "label": "film-hd21",
    "alt": "G-A3 film hd21",
    "code": "HD21"
  },
  {
    "src": "/images/products/dental-cabinet/g-a3/film-hc20.webp",
    "width": 500,
    "height": 500,
    "label": "film-hc20",
    "alt": "G-A3 film hc20",
    "code": "HC20"
  }
];
